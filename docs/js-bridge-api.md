...
未登录点击悬浮球：原生登录弹窗 → 登录接口返回并保存 `data.h5Url` 和原生长期 Token → 打开该 H5 首页。
已登录点击悬浮球：直接打开已保存的 `data.h5Url`。H5 首页和 Bridge 可信 Origin 不由第三方宿主配置，而是由登录接口下发的 URL 自动确定。悬浮球远程图片获取仍独立运行。
